...
/*
如果需要，将包含VA的块的内容刷新到磁盘，然后使用sys_page_map清除PTE_D位。
如果块不在块缓存中或者不是脏块，则不执行任何操作。
提示:请使用va_is_mapped、va_is_dirty和ide_write。
提示:在调用sys_page_map时使用pte_sycall常量。
提示:别忘了把地址四舍五入。
*/
void
flush_block(void *addr)
{
	...
	if( ( r = ide_write( blockno*8,addr,8) )<0 )
		panic("flush_block: ide_read error %e",r );
	if( ( r = sys_page_map(0,addr,0,addr,PTE_SYSCALL))<0 )
		panic("flush_blockL sys_page_map error %e",r );
}
