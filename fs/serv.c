...
/*
以req->req_omode模式打开req->req_path
分别存储在*pg_store和*perm_store中的Fd页面和返回调用环境的权限。
*/
int
...
// 将req->req_fileid的大小设置为req->req_size字节，截断或者根据需要扩展文件。
int
serve_set_size(envid_t envid, struct Fsreq_set_size *req)
{
	...
	/*
	每个文件系统IPC调用都有相同的一般结构。
	事情是这样的。
	首先，使用openfile_lookup查找相关的打开文件。
	失败时，使用ipc_send将错误代码返回给客户端。
	*/
...
/*
读取最多ipc->read.Req_n字节从当前seek position在ipc->read.req_fileid。
返回从文件中读取的字节给调用者(放在ipc->readRet)，然后更新seek position。
返回成功读取的字节数，错误时< 0。
	struct Fsreq_read {
		int req_fileid;
		size_t req_n;
...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	// 从req->fileid文件中读取req->req_n字节放在ret->ret_buf中, 并返回成功读取的字节数
	// ssize_t file_read(struct File *f, void *buf, size_t count, off_t offset)
	if ((r = openfile_lookup(envid, req->req_fileid, &o)) < 0)
		return r;
	// size_t count = MIN(req->req_n,o->o_file->f_size );
	if( (r = file_read( o->o_file,ret->ret_buf,req->req_n,o->o_fd->fd_offset) )<0 )
		return r;
	o->o_fd->fd_offset += r; // 修改偏移量
	return r;
}
...
/*
从req->req_buf写入req->req_n字节到req_fileid，从当前寻道位置开始，并相应地更新寻道位置。
