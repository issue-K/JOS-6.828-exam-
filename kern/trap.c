...
trap_init_percpu(void)
{
	...
	uint32_t CUR_TSS = (GD_TSS0 >> 3) + thiscpu->cpu_id;
	...
	gdt[CUR_TSS] = SEG16(STS_T32A, (uint32_t) (&thiscpu->cpu_ts),
...
page_fault_handler(struct Trapframe *tf)
{
    ...
    if( curenv->env_pgfault_upcall ) {
        struct UTrapframe *utf;  // 在异常栈中构建, 可以不使用汇编哦^_^
        if (tf->tf_esp >= UXSTACKTOP - PGSIZE && tf->tf_esp <= UXSTACKTOP - 1) {
            // 已经在异常栈中
            // 推入一个32位的空,再然后UTrapframe
        ...
        curenv->env_tf.tf_eip = (uint32_t) curenv->env_pgfault_upcall; // 跳转到页错误处理入口点
        env_run(curenv);  // 弹出env_tf中的所有寄存器开始执行
    }
