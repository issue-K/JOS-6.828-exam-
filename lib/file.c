...
/*
向文件服务器发送一个环境间请求，并等待回复。
请求体应该在fsipcbuf中，而响应可能被写回fsipcbuf。
type:请求类型，作为简单的整数IPC值传递。
...
open(const char *path, int mode)
{
	...
	// file descriptor.
/*
	使用fd_alloc查找未使用的文件描述符页。
	然后向文件服务器发送一个文件打开请求, 在请求中包含'path'和'omode'，
	并将返回的文件描述符页映射到适当的fd地址。
	FSREQ_OPEN成功返回0，失败返回< 0。

	(fd_alloc并不分配一个页面，它只是返回一个未使用的fd地址。你需要分配一个页面吗?)

	返回文件描述符索引。
	如果fd_alloc之后的任何步骤失败，请使用fd_close释放文件描述符。
*/
	int r;
	struct Fd *fd;
...
注意:fsipcbuf.write.Req_buf只有这么大，
但请记住，写操作总是允许写入比请求的字节更少的字节。	
