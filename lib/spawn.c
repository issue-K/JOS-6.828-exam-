...
从文件系统加载的程序映像生成一个子进程。
Prog:要运行程序的路径名。
Argv:指向以空字符结尾的字符串指针数组的指针，它将作为命令行参数传递给子进程。
成功时返回子节点envid，失败时返回< 0
*/
int
...
init_stack(envid_t child, const char **argv, uintptr_t *init_esp)
{
	...
	/*
	确定字符串和argv数组的位置。
	设置指向临时页面“UTEMP”的指针;稍后我们将在那里映射一个页面，然后将该页面重新映射到子环境(USTACKTOP - PGSIZE)。
	Strings是栈上最上面的东西。	
	*/
	...
	if ((r = sys_page_alloc(0, (void*) UTEMP, PTE_P|PTE_U|PTE_W)) < 0)
		...
		/*
初始化'argv_store[i]'指向参数字符串i，对于所有0 <= i < argc。同样，将参数字符串从'argv'复制到新分配的堆栈页面中。

*设置'argv_store[argc]'为0以null终止args数组。

*在“args”下面再向孩子的堆栈中添加两个单词，
包含argc和argv参数，传递给子进程的umain()函数。Argv应该在堆栈上的argc下面。(同样，argv应该使用在子环境中有效的地址。)
...
copy_shared_pages(envid_t child)
{
	...
	int r;
	for(;i<UTOP;i+=PGSIZE){
		bool exist = ( uvpd[PDX(i)] & PTE_P ) && ( uvpt[PGNUM(i)] & PTE_P );
		if( !exist )	continue;
		bool share = uvpt[PGNUM(i)] & PTE_SHARE;
		bool user = uvpt[PGNUM(i)] & PTE_U;
		if( share && user ){
